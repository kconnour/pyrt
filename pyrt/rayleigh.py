...
from pyrt.spectral import wavenumber as wavenum
...
def rayleigh_co2_optical_depth(
        column_density: np.ndarray, wavelength: ArrayLike) -> np.ndarray:
    r"""Compute the Rayleigh CO :sub:`2` scattering optical depth in each layer
    given its column density at a range of input wavelengths.
    ...
    -------
    np.ndarray
        The Rayleigh scattering optical depth for CO :sub:`2`.
    ...
    >>> rayleigh_od = pyrt.rayleigh_co2_optical_depth(column_density, wavs)
    >>> rayleigh_od.shape
    (15, 5)
